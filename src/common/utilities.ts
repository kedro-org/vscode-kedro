...
export async function discoverKedroProjects(): Promise<KedroProjectInfo[]> {
    // vscode.workspace.findFiles doesn't respect .gitignore, which causes it to scan
    // node_modules, .venv, and other ignored directories — producing false positives.
    // findFiles2 (which exposes a useIgnoreFiles option) was proposed in VS Code 1.85
    // but never made it into the stable public API or @types/vscode.
    // globby is already a transitive dependency and has first-class .gitignore support
    // via { gitignore: true }, making it the cleanest available solution.
    const workspaceFolders = getWorkspaceFolders();
    const seen = new Set<string>();
    const projects: KedroProjectInfo[] = [];

    for (const folder of workspaceFolders) {
        const files = await globby('**/pyproject.toml', {
